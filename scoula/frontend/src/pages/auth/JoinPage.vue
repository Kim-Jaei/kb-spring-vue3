...
const checkError = ref(''); // 초기값은 빈 문자열
...
const join = async () => {
  ...
  try {
    ...
    router.push({ name: 'home' }); // 회원가입 성공 시, 첫 페이지 or 로그인 페이지로 이동
  } catch (e) {
    console.error(e);
  }
};
...
<template>
  <div class="mt-5 mx-auto" style="width: 500px">
    ...
    <form @submit.prevent="join">
      <!-- @submit.prevent="join" 전송 눌렀을 때 서버에 직접전송X, join 함수 호출 -->
      <div class="mb-3 mt-3">
        ...
        />
        <!-- @input = "changeUsername" 입력할 때마다 함수 호출 -->
      </div>
      <div>
