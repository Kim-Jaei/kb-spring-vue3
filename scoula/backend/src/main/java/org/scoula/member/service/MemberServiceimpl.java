...
public class MemberServiceimpl implements MemberService {
    final PasswordEncoder passwordEncoder; // SecurityConfig - Bean 등록을 주입 받음
    final MemberMapper mapper;

    @Override
    public boolean checkDuplicate(String username) {
        MemberVO member = mapper.findByUsername(username);
        return member != null ? true : false; // true면 중복
    }
    ...
    private void saveAvatar(MultipartFile avatar, String username) {
        // 아바타 업로드
        // 아바타 이미지 : 헤더(파일 식별 넘버) + 그림 데이터
        if (avatar != null && !avatar.isEmpty()) {
            File dest = new File("c:/upload/avatar", username + ".png"); // 헤더 고정
            try{
    ...
    public MemberDTO join(MemberJoinDTO dto) {
        ...
        return get(member.getUsername()); // join 발생!
    }
